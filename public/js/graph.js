  ...
  let graph = new Vue({
    ...
    data: {
      ...
      optionsHumidity: {
        ...
        graph: {
          type: 'polyline',
          classStyle: 'graphLine_Diagram blueDiagram',
        },
        id: 'humidity',
        size_px: { x: 1000, y: 500 },
        axis: {
    ...
    template: `<section>
                <div id="dateList_id">
                    <ul>
                        ...
                        <li>
                            <ul>
                                <li class="id_captor_li" v-for="index in 6">
    ...
    mounted() {
      ...
      this.optionsTemperature.horizontalLines = this.generateHorizontalLines();
      this.updateGraph();
    },
    computed: {
      temperatureValues() {
    ...
    methods: {
        ...
        for (let captorId in this.measures) {
          ...
          for (let measureId in data) {
            ...
            if (this.isOnlyDate(measure.date)) {
              this.measures[captorId].data[measureId].dateTime = new Date(
                measure.date + ' 14:00:00'
              ).getTime();
            } else {
              let date = new Date(measure.date);
              date.setHours(date.getHours() + 2);
              this.measures[captorId].data[measureId].dateTime = date.getTime();
            }
          }
        }
      ...
      generateHorizontalLines() {
        ...
        for (let t = 0; t <= 40; t += 2) {
          if (t % 10 === 0) {
            lines.push({ position: t, classStyle: 'strongLine_Diagram' });
          } else {
            lines.push({ position: t, classStyle: 'lightLine_Diagram' });
          }
        }
        return lines;
      },
