  ...
  let panel = new Vue({
    ...
    methods: {
      ...
      temperature(idCaptor) {
        return (this.lastMeasure(idCaptor).temperature / 10).toFixed(2);
      },
      humidity(idCaptor) {
        return (this.lastMeasure(idCaptor).humidity / 10).toFixed(2);
      },
      numberOfMeasures(idCaptor) {
        return this.measures[idCaptor].data.length;
      },
      lastMeasure(idCaptor) {
        return this.measures[idCaptor].data[
          this.numberOfMeasures(idCaptor) - 1
        ];
      },
      name(idCaptor) {
